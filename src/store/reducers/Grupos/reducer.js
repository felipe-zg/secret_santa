...
export default function Grupos(state=[], action){
    switch(action.type){
        ...
        case 'ATUALIZAR_GRUPO':{
            return produce(state, draft=>{
                const groupIndex = 
                  draft.findIndex(g=> g.id === action.grupo.id);
        ...
        case 'EXCLUIR_GRUPO':{
            return produce(state, draft=>{
                const groupIndex = 
                  draft.findIndex(g=> g.id === action.id);
        ...
        case 'EXCLUIR_PARTICIPANTE':{
            return produce(state, draft=>{
                const groupIndex = 
                  draft.findIndex(g=> g.id === action.idGrupo);
                const participantIndex = 
                    draft[groupIndex].participantes.findIndex(p=> p.id === action.idParticipante);
        ...
        case 'ATUALIZAR_PARTICIPANTE':{
            return produce(state, draft=>{
                const groupIndex = 
                  draft.findIndex(g=> g.id === action.idGrupo);
                const participantIndex = 
                    draft[groupIndex].participantes.findIndex(p=> p.id === action.participante.id);
        ...
        case 'ADICIONAR_PARTICIPANTE':{
            return produce(state, draft=>{
                const groupIndex = 
                  draft.findIndex(g=> g.id === action.idGrupo);
                draft[groupIndex].participantes = [...draft[groupIndex].participantes, action.participante];
            })
        }
        default:{
            return state;
