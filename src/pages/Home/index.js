...
export default function Home({navigation}) {
  ...
  useEffect(() => {
    ...
    buscaGrupos().then(gruposExistentes =>
      dispatch(GrupoActions.iniciarListaDeGrupos(gruposExistentes)),
    );
  }, [dispatch]);
        ...
        {grupos.length > 0 ? (
          <Grupos
            ...
            renderItem={({item}) => (
              <Grupo
                onPress={() =>
                  navigation.navigate('TelaDetalhesDoGrupo', {id: item.id})
                }>
      ...
      <ViewAcoes>
        <Acoes>
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('TelaPoliticaDePrivacidade')}>
            <Text style={styles.txtPolitica}>Política de privacidade</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.politica}
            onPress={() => navigation.navigate('TelaContato')}>
            <Text style={styles.txtPolitica}>Fale conosco</Text>
          </TouchableOpacity>
        </Acoes>
      </ViewAcoes>
    </Container>
